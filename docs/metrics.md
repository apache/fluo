...
A Fluo application can be configured (in [application.properties]) to report metrics. When metrics are
...
by [application.properties] and are described in detail below.

## Configuring reporters

Fluo metrics are not published by default. To publish metrics, configure a reporter in the 'metrics'
section of [application.properties]. There are several different reporter types (i.e Console, CSV,
