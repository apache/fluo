...
start-yarn)
  java -cp "$FLUO_HOME/lib/*:$FLUO_HOME/lib/logback/*" io.fluo.yarn.OracleApp -fluo-home $FLUO_HOME -hadoop-prefix $HADOOP_PREFIX
	;;
start-local)
  java -cp "$FLUO_HOME/lib/*" io.fluo.yarn.OracleRunnable -config-dir $FLUO_CONF_DIR -log-output $FLUO_LOG_DIR >${FLUO_LOG_DIR}/${SERVICE}_${LOGHOST}.out 2>${FLUO_LOG_DIR}/${SERVICE}_${LOGHOST}.err &
	;;
stop-local)
