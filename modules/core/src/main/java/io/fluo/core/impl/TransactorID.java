...
public class TransactorID implements Closeable {
  ...
  private PersistentEphemeralNode createEphemeralNode() throws InterruptedException {
    @SuppressWarnings("resource")
    PersistentEphemeralNode result = new PersistentEphemeralNode(env.getSharedResources().getCurator(),
        Mode.EPHEMERAL, getNodePath(), getStringID().getBytes());
    result.start();
    int waitTime = 0;
    while (result.waitForInitialCreate(1, TimeUnit.SECONDS) == false) {
      waitTime += 1;
