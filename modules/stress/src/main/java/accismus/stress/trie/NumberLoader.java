...
public class NumberLoader implements Loader {
  ...
  public void load(Transaction tx) throws Exception {
    ...
    Integer seen = ttx.get().row(rowId).col(COUNT_SEEN_COL).toInteger(0);
    if (seen == 0) {
      Integer wait = ttx.get().row(rowId).col(COUNT_WAIT_COL).toInteger(0);
      if (wait == 0) {
        ttx.mutate().row(rowId).col(COUNT_WAIT_COL).set(1);
      }
    }
  }
}
